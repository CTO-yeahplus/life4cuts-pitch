...
const SLIDES = [
  ...
  { id: "03", icon: <Crown className="w-12 h-12 text-pink-500 mb-6" />, title: "핵심 경쟁력", subtitle: "Core Competencies", content: ( <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center"><div className="space-y-6 text-lg text-gray-300"><p>자본 투자의 가장 큰 적은 '유행의 증발'입니다. 인생네컷 플러스는 압도적인 IP 파워로 유행을 타지 않는 구조를 완성했습니다.</p><ul className="space-y-5">{["프리미엄 앵커 테넌트 (반다이 남코 등 프리미엄 기기 유치)", "독점 K-IP 라인업 (카카오프렌즈, 국내 웹툰 IP 독점 공급)", "객단가(ATV)의 퀀텀 점프 (사진 촬영 유입 ➡️ 가챠 추가 지출 유도)"].map((text, idx) => (<li key={idx} className="flex items-start gap-3"><CheckCircle2 className="w-6 h-6 text-green-500 flex-shrink-0 mt-1" /><span>{text}</span></li>))}</ul></div><div className="w-full h-[300px] md:h-[400px] rounded-[2rem] bg-zinc-900 border border-white/10 flex flex-col items-center justify-center p-8 text-center relative overflow-hidden group shadow-2xl"><div className="absolute inset-0 bg-black/40 group-hover:bg-black/10 transition-colors duration-500" /><Users className="w-16 h-16 text-zinc-700 mb-6 relative z-10" /><div className="absolute bottom-6 left-6 right-6 p-4 rounded-xl bg-black/80 backdrop-blur-md border border-white/10 relative z-10"><h4 className="font-bold text-lg text-white mb-1">BANDAI NAMCO Direct Asset</h4><p className="text-xs text-gray-400">화려하게 세팅된 기기 매장에 줄 서 있는 고객들 (이미지 영역)</p></div></div></div> ) },
